...
export function RegisterForm({ onRegisterSuccess, onSwitchToLogin }: RegisterFormProps) {
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            ...
            // Giả sử Backend trả về { token: "...", id: "...", username: "...", roles: [...] }
            const responseData = response.data;

            alert("Xác thực thành công!");

            // ✅ TRUYỀN ĐÚNG DỮ LIỆU TỪ BACKEND
            if (responseData.token) {
                onRegisterSuccess(responseData.token, responseData);
            } else {
                // Nếu verify-otp chỉ trả về message, bạn có thể chuyển hướng về login
            ...
            setErrorMessage(error.response?.data?.message || error.response?.data || "Mã OTP sai hoặc hết hạn!");
        } finally {
            setIsLoading(false);
        }
    };
